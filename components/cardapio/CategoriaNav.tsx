...
export function CategoriaNav({ categorias, activeCategory, onSelect }: CategoriaNavProps) {
  return (
    <nav className="sticky top-[70px] z-[90] border-b border-vino-border bg-vino-surface">
      <div className="no-scrollbar mx-auto flex max-w-[1100px] overflow-x-auto px-4 md:px-8">
        {categorias.map((categoria) => (
          <button
            key={categoria.id}
            className={`shrink-0 border-b-2 px-5 py-4 text-xs uppercase tracking-[0.15em] transition ${
